...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     \section{Enunciado}
     Calcular la intensidad que circula por la resistencia de $\qty{30}{\ohm}$ del circuito de la figura aplicando el principio de
     superposición.
...
las fuentes de tensión y dejando en circuito abierto la fuente de
corriente). Se analiza cada circuito por separado.

\vspace{2mm}
\underline{Actúa solo la fuente de tensión de $\qty{32}{\volt}$}
...
\vspace{2mm}
\underline{Actúa solo la fuente de tensión de $\qty{64}{\volt}$}
...
\end{equation*}
Aplicando divisor de corriente, la corriente $I''$ (la que
circula por la resistencia de 30 $\Omega$ en este caso) es:
\begin{equation*}
...
\vspace{2mm}
\underline{Actúa solo la fuente de corriente de $\qty{4}{\ampere}$}
...
Dado que la corriente total del circuito son 4 A, aplicando divisor de corriente, la corriente $I'''$ (la que circula por la
...
30 $\Omega$ es:
\begin{equation*}
  I=I'+I''+I'''=0.4+0.8+1=\boxed{\qty{2.2}{\ampere}}
\end{equation*}
...
circula corriente (circuito abierto entre $A-B$), el potencial en $B$
es:
\begin{equation*}
  U_B=\qty{16}{\volt}
\end{equation*}
...
\end{equation*}
por lo que el potencial en el punto $A$:
\begin{equation*}
...
El equivalente de Thévenin es por tanto:

\begin{center}
  \includegraphics{figuras/BT1_17_th.pdf}
\end{center}

Aplicando el $\textrm{T}^\textrm{a}$ de máxima transferencia de potencia, la resistencia a conectar es:
\begin{equation*}
  R_L=R_{th}=\qty{4}{\ohm}
\end{equation*}
...
En el circuito de la figura, se debe calcular el equivalente de Norton entre terminales A-B.

    \begin{minipage}{0.6\linewidth}
      \includegraphics{figuras/BT1_ej19_enunciado.pdf}
    \end{minipage}
    \begin{minipage}{0.4\linewidth}
      Datos:
      \vspace{2mm}
    ...
    Para calcular $I_N$, hay que cortocircuitar los terminales A-B y calcular la corriente de cortocircuito:

    \begin{center}
        \includegraphics[scale=1]{figuras/BT1_ej19_SC.pdf}
    \end{center}      
      
      Dado que únicamente hay 2 nudos, solo es necesaria una ecuación para resolver. 
      Definiendo el nudo C, y tomando B como referencia, aplicamos 1LK en C:
      \begin{equation*}
          \alpha \cdot U_R + I_g - U_C \cdot \bigg( \hspace{-6mm} \underbrace{\frac{1}{R_1} + \frac{1}{R_3}}_{\hspace{8mm} = \frac{1}{1} + \frac{1}{1} = 2\,\si{\ohm}^{-1}} \hspace{-6mm} \bigg) = 0
      \end{equation*}
    
      donde $U_R=U_C$, luego:
      \begin{equation*}
          I_g = U_C \cdot (2-\alpha) \quad \rightarrow \quad I_{SC} = \frac{U_C}{R_3} = \frac{I_g / 1.5}{R_3} = \boxed{\qty{4}{\ampere} = I_N}
      \end{equation*}
      
    \vspace{2mm}
    Para calcular $R_N$, se apagan las fuentes independientes y se conecta un generador de prueba entre A y B:
    \begin{center}
      \includegraphics[scale=1]{figuras/BT1_ej19_fuentePrueba.pdf}
    \end{center}
            
    \vspace{2mm}
    Aplicando 2LK:
    \begin{equation*}
        \epsilon_0 = I_0\cdot R_3 + I_1\cdot R_1 
        \overarrow[=][\downarrow]{\footnotesize {$R_1=R_3=\qty{1}{\ohm}$} }
        I_0 + I_1        
    \end{equation*}
    % Flecha, sacada de aquí: https://tex.stackexchange.com/questions/8720/overbrace-underbrace-but-with-an-arrow-instead

    \vspace{3mm}
    Y aplicando 1LK en C':
    \begin{equation*}
        \alpha \cdot U_R -I_1 + I_0 = 0 
        \; 
        \overarrow[=][\downarrow]{\footnotesize {$U_R = -I_0 \cdot R_3 = -I_0$} }
        \;
        \alpha \cdot (-I_0) -I_1 + I_0
    \end{equation*}    
    
    \vspace{3mm}
    Finalmente, sustituyendo la primera ecuación ($I_1 = \epsilon_0 - I_0$) en la segunda:
    
    \begin{equation*}
        - \alpha \cdot I_0 - \epsilon_0 + I_0 + I_0 = 0 \quad \rightarrow \quad \frac{\epsilon_0}{I_0} = \boxed{\frac{3}{2}\,\si{\ohm} = R_N}
    \end{equation*}
